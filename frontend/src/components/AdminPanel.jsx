...
import { Plus, Trash2, Code, FileText, TestTube, Save, ArrowLeft, Crown, X, Users, Package } from 'lucide-react';
import { AlertCircle, CheckCircle } from 'lucide-react';
import { useNavigate } from 'react-router';
import axiosClient from '../utils/axiosClient.js';
import { fetchAdminProblemList } from '../utils/fetchAdminProblemList.js';
...
function AdminPanel() {
  ...
  useEffect(() => {
    if (showContestModal) {
      fetchAdminProblemList().then(setProblemsList);
    }
  }, [showContestModal]);

  const handleContestFormChange = (e, idx) => {
    const { name, value } = e.target;
    if (name.startsWith('problemId')) {
      const newProblems = [...contestForm.problems];
      newProblems[idx].problemId = value;
      setContestForm({ ...contestForm, problems: newProblems });
    } else if (name.startsWith('marks')) {
      const newProblems = [...contestForm.problems];
      newProblems[idx].marks = Number(value);
      setContestForm({ ...contestForm, problems: newProblems });
    } else {
      setContestForm({ ...contestForm, [name]: value });
    }
  };
  ...
  const handleCreateContest = async () => {
    ...
    setContestError('');
    try {
      await axiosClient.post('/contest/create', contestForm);
      setShowContestModal(false);
      setContestForm({
        name: '', description: '', instructions: '', startTime: '', endTime: '',
        problems: [ { problemId: '', marks: 100 }, { problemId: '', marks: 150 }, { problemId: '', marks: 200 } ]
      });
    } catch (err) {
    ...
    try {
      ...
      if (error.response?.data?.errors) {
        ...
        const errorMessages = error.response.data.errors.map(err => 
          `${err.language}: ${err.status}${err.stderr ? ` - ${err.stderr}` : ''}`
        ).join('\n');
        setCreateError(`Reference solution validation failed:\n${errorMessages}`);
      } else {
        setCreateError(error.response?.data?.message || 'Failed to create problem. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <LoadingSpinner size="xl" text="Creating problem..." />
      </div>
    );
  }
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold gradient-text">Create New Problem</h1>
            </div>
            <div className="flex items-center space-x-4">
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          {createSuccess && (
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <span className="text-green-400">Problem created successfully! Redirecting...</span>
              </div>
            </motion.div>
          )}
          ...
          {createError && (
            ...
            >
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <div className="text-red-400 font-semibold mb-2">Error Creating Problem</div>
                  <pre className="text-red-300 text-sm whitespace-pre-wrap font-mono bg-red-900/20 p-3 rounded border border-red-500/30">
                    {createError}
                  </pre>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
            {/* Existing cards can go here */}
            {/* Removed Customer Orders and Make Contest cards as they are now on separate pages */}
          </div>
          {/* Tab Navigation */}
          <motion.div
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            >
              ...
              >
                ...
                {createSuccess ? 'Created!' : 'Create Problem'}
              </GradientButton>
            </motion.div>
          </form>
        </div>
